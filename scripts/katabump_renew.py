...
def update_github_secret(secret_name: str, secret_value: str) -> bool:
    ...
    if not REPO_TOKEN or not GITHUB_REPOSITORY:
        log('未配置 REPO_TOKEN 或 GITHUB_REPOSITORY，跳过更新 Secret', 'WARNING')
        return False
    ...
    try:
        ...
        if resp.status_code != 200:
            log(f'获取公钥失败: {resp.status_code} {resp.text}', 'ERROR')
            return False
        ...
        if resp.status_code in (201, 204):
            log(f'成功更新 Secret: {secret_name}', 'SUCCESS')
            return True
        else:
            log(f'更新 Secret 失败: {resp.status_code} {resp.text}', 'ERROR')
            return False
...
class KataBumpRenewer:
    def __init__(self):
        ...
        self.last_html = ''
        self.new_cookies = None  # 存储新的 cookies
        ...
        # 使用 Session 来自动处理 cookies
        self.session = requests.Session()
        
        # 解析初始 cookies 并设置到 session
        if KATA_COOKIES:
    ...
    def get(self, path, json_response=False):
        ...
        del headers['cookie']  # 使用 session 管理 cookies
    ...
    def save_cookies(self):
        """保存更新后的 cookies 到 GitHub Secrets"""
        new_cookies = self.get_current_cookies()
        
        if not new_cookies:
            log('没有 cookies 需要保存', 'WARNING')
            return
        
        # 检查 cookies 是否有变化
        if new_cookies == KATA_COOKIES:
            log('Cookies 未变化，无需更新', 'DEBUG' if DEBUG_MODE else 'INFO')
            return
        
        log('检测到 Cookies 变化，尝试更新 Secret...')
        
        if update_github_secret('KATA_COOKIES', new_cookies):
            log('Cookies 已更新到 GitHub Secrets', 'SUCCESS')
        else:
            log('Cookies 更新失败', 'WARNING')
